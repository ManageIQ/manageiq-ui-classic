describe TreeBuilderInstances do
  ...
  it 'sets availability zones correctly if vms are hidden' do
    provider_with_az = @instances_tree.x_get_tree_roots(false)[0] # provider with vm that has availability zone
    provider_without_az = @instances_tree.x_get_tree_roots(false)[1] # provider with vm that doesn't have availability zone
