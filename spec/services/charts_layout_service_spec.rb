describe ChartsLayoutService do
  ...
  let(:host_openstack_infra_chart) do
    YAML.safe_load(File.read("#{File.join(ApplicationController::Performance::CHARTS_LAYOUTS_FOLDER, 'daily_perf_charts', 'ManageIQ_Providers_Openstack_InfraManager_Host')}.yaml"))
  end
  let(:host_chart) do
    YAML.safe_load(File.read("#{File.join(ApplicationController::Performance::CHARTS_LAYOUTS_FOLDER, 'daily_perf_charts', 'Host')}.yaml"))
  end
  let(:layout_chart) do
    YAML.safe_load(File.read("#{File.join(ApplicationController::Performance::CHARTS_LAYOUTS_FOLDER, 'daily_util_charts')}.yaml"))
  end
