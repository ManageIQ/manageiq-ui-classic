describe OpsController do
  before do
    EvmSpecHelper.seed_specific_product_features(
      %w(vm vm_compare vm_delete instance instance_delete image image_delete miq_template
         miq_template_delete configuration_manager_explorer configuration_manager_view)
    )
  end
  ...
  describe '#recurse_sections_and_features' do
    ...
    context '"_tab_conf" feature node' do
      it 'yields features including "configuration_manager_view"' do
        expect do |b|
          controller.send(:recurse_sections_and_features, '_tab_conf', &b)
        end.to yield_successive_args(["configuration_manager_explorer", include("configuration_manager_view")])
      end
    end
