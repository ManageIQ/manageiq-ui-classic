describe MiqAeClassController do
  ...
  describe "#copy_objects" do
    ...
    describe "#copy_objects_save" do
      ...
      def build_copy_edit(opts = {})
        new_params = {
          :domain            => opts.fetch(:domain, @d2.id),
          :namespace         => opts.fetch(:namespace, @ns2.fqname),
          :override_existing => opts.fetch(:override_existing, false),
          :new_name          => opts.fetch(:new_name, nil)
        }
        {
      ...
      def setup_session(edit)
        session[:edit] = edit
        session[:sandboxes] = {"miq_ae_class" => {:action => "miq_ae_class_copy"}}
      end

      it "copies class to a different namespace" do
        edit = build_copy_edit
        setup_session(edit)

        allow(controller).to receive(:build_automate_tree)

        expect do
          post :copy_objects_save, :params => {:id => @cls1.id}
        end.to change(MiqAeClass, :count).by(1)

        json_response = response.parsed_body
        expect(json_response["message"]).to include("Copy selected Automate Class was saved")
        copied_cls = MiqAeClass.find_by(:name => @cls1.name, :namespace_id => @ns2.id)
        expect(copied_cls).not_to be_nil
        expect(controller.send(:x_node)).to eq("aec-#{copied_cls.id}")
      end

      it "returns error when class already exists without overwrite flag" do
        FactoryBot.create(:miq_ae_class, :name => @cls1.name, :namespace_id => @ns2.id)
        edit = build_copy_edit(:override_existing => false)
        setup_session(edit)
      ...
      it "overwrites existing class when override_existing is true" do
        FactoryBot.create(:miq_ae_class, :name => @cls1.name, :namespace_id => @ns2.id)
        edit = build_copy_edit(:override_existing => true)
        setup_session(edit)
      ...
      it "copies class with a new name under the same domain" do
        edit = build_copy_edit(:domain => @d1.id, :namespace => @ns1.fqname,
                               :override_existing => true, :new_name => "cls1_copy")
        setup_session(edit)
        ...
        json_response = response.parsed_body
        expect(json_response["message"]).to include("Copy selected Automate Class was saved")
        expect(MiqAeClass.find_by(:name => "cls1_copy", :namespace_id => @ns1.id)).not_to be_nil
      end
    end
  end
  ...
  describe "Instance Workflows" do
    ...
    describe "#instance_form_data" do
      context "for new instance" do
        ...
        it "returns an error when class_id parameter is missing for new instance" do
    ...
    describe "#create_instance" do
      it "creates a new instance with valid parameters" do
        ...
        end.to change(MiqAeInstance, :count).by(1)
      ...
      it "handles database errors gracefully" do
        ...
        fake_instance = instance_double(MiqAeInstance)
        allow(fake_instance).to receive(:name=)
        allow(fake_instance).to receive(:display_name=)
        allow(fake_instance).to receive(:description=)
        allow(fake_instance).to receive(:class_id=)
        allow(fake_instance).to receive(:ae_values=)
        allow(fake_instance).to receive(:ae_values).and_return([])
        allow(fake_instance).to receive(:save!).and_raise(StandardError.new("Database error"))
        allow(MiqAeInstance).to receive(:new).and_return(fake_instance)
    ...
    describe "#update_instance" do
      ...
      it "updates an existing instance with valid parameters" do
        ...
        post :update_instance, :params => params
      ...
      it "handles database errors gracefully" do
        ...
        allow(@ae_instance).to receive(:save!).and_raise(StandardError.new("Update error"))
        allow(controller).to receive(:find_record_with_rbac).and_call_original
        allow(controller).to receive(:find_record_with_rbac).with(MiqAeInstance, @ae_instance.id.to_s).and_return(@ae_instance)
  ...
  describe "Copy Workflows" do
    before do
      ...
      @ae_instance = FactoryBot.create(:miq_ae_instance, :name => "test_instance", :class_id => @ae_class.id)
      @ae_method = FactoryBot.create(:miq_ae_method, :name => "test_method", :class_id => @ae_class.id, :scope => "class", :language => "ruby")
      controller.instance_variable_set(:@sb, {})
    end
    ...
    describe "#copy_objects_save" do
      ...
      context "copying a method" do
        it "successfully copies method to different class" do
          FactoryBot.create(:miq_ae_class, :name => "target_class", :namespace_id => @namespace2.id)
          selected_items = {@ae_method.id => @ae_method.name}
          edit = {
            :key            => "copy_objects__#{@ae_method.id}",
            :new            => {
              :domain            => @domain2.id,
              :namespace         => @namespace2.fqname,
              :override_existing => false,
          ...
          controller.params = {:id => @ae_method.id}
          controller.instance_variable_set(:@sb, :action => "miq_ae_method_copy")
          controller.instance_variable_set(:@edit, edit)
          session[:edit] = edit
          ...
          expect do
            controller.send(:copy_objects_save)
          end.to change(MiqAeMethod, :count).by(1)
