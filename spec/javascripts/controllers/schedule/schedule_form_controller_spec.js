describe('scheduleFormController', function() {
  var $scope, vm, $httpBackend, miqService, timerOptionService, oneMonthAgo;
  ...
  beforeEach(inject(function($rootScope, _$controller_, _$httpBackend_, _miqService_, _timerOptionService_) {
    ...
    vm = _$controller_('scheduleFormController', {
      $scope: $scope,
      scheduleFormId: 'new',
  ...
  var sharedBehaviorForInitialization = function() {
    ...
    it('sets the action type to the type returned from the http request', function() {
      expect(vm.scheduleModel.action_typ).toEqual('actionType');
    });

    it('sets the depot name', function() {
      expect(vm.scheduleModel.depot_name).toEqual('depotName');
    });

    it('sets the logUserid to the log_userid returned from the http request', function() {
      expect(vm.scheduleModel.log_userid).toEqual('logUserId');
    });

    it('sets the logPassword to the log_password returned from the http request', function() {
      expect(vm.scheduleModel.log_password).toEqual(miqService.storedPasswordPlaceholder);
    });

    it('sets the scheduleName to the name returned from the http request', function() {
      expect(vm.scheduleModel.name).toEqual('scheduleName');
    });

    it('sets the scheduleDescription to the description returned from the http request', function() {
      expect(vm.scheduleModel.description).toEqual('scheduleDescription');
    });

    it('sets the scheduleEnabled to the enabled attribute returned from the http request', function() {
      expect(vm.scheduleModel.enabled).toEqual(true);
    });

    it('sets the scheduleTimerType', function() {
      expect(vm.scheduleModel.timer_typ).toEqual('Hourly');
    });

    it('sets the scheduleTimerValue', function() {
      expect(vm.scheduleModel.timer_value).toEqual('8');
    });

    it('sets the scheduleDate', function() {
      expect(vm.scheduleModel.start_date).toEqual(moment('01/01/2015').format('MM/DD/YYYY'));
    });

    it('sets the scheduleStartHour', function() {
      expect(vm.scheduleModel.start_hour).toEqual('12');
    });

    it('sets the scheduleStartMinute', function() {
      expect(vm.scheduleModel.start_min).toEqual('25');
    });

    it('sets the scheduleTimeZone', function() {
      expect(vm.scheduleModel.time_zone).toEqual('UTC');
    });

    it('sets the uri', function() {
      expect(vm.scheduleModel.uri).toEqual('uri');
    });

    it('sets the uriPrefix', function() {
      expect(vm.scheduleModel.uri_prefix).toEqual('uriPrefix');
    });

    it('sets the timer items', function() {
      expect(vm.timer_items).toEqual(["some", "options"]);
    });
  ...
  describe('initialization', function() {
    describe('when the scheduleFormId is new', function() {
      ...
      it('sets the action type to vm', function() {
        expect(vm.scheduleModel.action_typ).toEqual('vm');
      });

      it('sets the filter type to all', function() {
        expect(vm.scheduleModel.filter_typ).toEqual('all');
      });

      it('sets the filterValuesEmpty to true', function() {
        expect(vm.filterValuesEmpty).toBe(true);
      });

      it('sets the scheduleDate to today', function() {
        expect(vm.scheduleModel.start_date).toEqual(moment("01/02/2014").format('MM/DD/YYYY'));
      });

      it('sets the scheduleTimerType to once', function() {
        expect(vm.scheduleModel.timer_typ).toEqual('Once');
      });
      ...
      it('sets the scheduleStartHour to 0', function() {
        expect(vm.scheduleModel.start_hour).toEqual('0');
      });

      it('sets the scheduleStartMinute to 0', function() {
        expect(vm.scheduleModel.start_min).toEqual('0');
      });
    ...
    describe('when the scheduleFormId is an id', function() {
      ...
      describe('when the filter type is all', function() {
        beforeEach(inject(function(_$controller_) {
          ...
          vm = _$controller_('scheduleFormController', {$scope: $scope, storageTable: 'Potatostore', scheduleFormId: '12345', oneMonthAgo: oneMonthAgo});
          $httpBackend.flush();
        }));
      ...
      describe('when the protocol exists', function() {
        ...
        it('sets the log protocol', function() {
          expect(vm.scheduleModel.log_protocol).toBe('protocol')
        });
      });
      ...
      describe('when the filter type is not all', function() {
        beforeEach(inject(function(_$controller_) {
          ...
          vm = _$controller_('scheduleFormController', {$scope: $scope, scheduleFormId: '12345', oneMonthAgo: oneMonthAgo});
          $httpBackend.flush();
        }));
        ...
        it('sets the filter type to the type returned from the http request', function() {
          expect(vm.scheduleModel.filter_typ).toEqual('filterType');
        });

        it('sets the filter list', function() {
          expect(vm.filterList).toEqual([{text: 'lol', value: 'lol'}, {text: 'lol2', value: 'lol2'}]);
        });

        it('sets the filter value to the value returned from the http request', function() {
          expect(vm.scheduleModel.filter_value).toEqual('filterValue');
        });

        it('sets the filterValuesEmpty to false', function() {
          expect(vm.filterValuesEmpty).toBe(false);
        });
      });
    });
  ...
  describe('#determineActionType', function() {
    describe('when the action type is a vm type', function() {
      beforeEach(function() {
        vm.scheduleModel.action_typ = 'vm_potato';
      });

      it('returns vm', function() {
        expect(vm.determineActionType()).toEqual('vm');
      });
    });
    ...
    describe('when the action type is a host type', function() {
      ...
      it('returns host', function() {
        expect(vm.determineActionType()).toEqual('host');
      });
    });

    describe('when the action type is any other type', function() {
      beforeEach(function() {
        vm.scheduleModel.action_typ = 'potato';
      });

      it('returns that type', function() {
        expect(vm.determineActionType()).toEqual('potato');
      });
    });
  });
  ...
  describe('#filterTypeChanged', function() {
    describe('when the filter type is all', function() {
      ...
      it('sets the filterValue to ""', function() {
        expect(vm.scheduleModel.filter_value).toEqual("");
      });
    });

    describe('when the filter type is not all', function() {
      beforeEach(function() {
        vm.scheduleModel.filter_typ = 'not_all';
        $scope.angularForm = {
          filter_value: {
            $name:       'filter_value',
            $setViewValue: function (value){}
      ...
      describe('when the ajax request returns with success', function() {
        ...
        describe('when the item list is a multi dimension array', function() {
          ...
          it('creates a new drop down with the list response', function() {
            expect(vm.filterList).toEqual([{value: 'lolvalue', text: 'loloption'}, {value: 'lol2value', text: 'lol2option'}]);
          });
  ...
  describe('#logProtocolChanged', function() {
    describe('when the log protocol is samba', function() {
      beforeEach(function() {
        vm.scheduleModel.log_protocol = 'Samba';
      });

      it('sets the uriPrefix to smb', function() {
        vm.logProtocolChanged();
        expect(vm.scheduleModel.uri_prefix).toEqual('smb');
      });
    });
    ...
    describe('when the log protocol is network file system', function() {
      ...
      it('sets the uriPrefix to nfs', function() {
        vm.logProtocolChanged();
        expect(vm.scheduleModel.uri_prefix).toEqual('nfs');
      });
    });
  });
  ...
  describe('#actionTypeChanged', function() {
    describe('when the action type is db_backup', function() {
      beforeEach(function() {
        vm.scheduleModel.action_typ = 'db_backup';
        vm.actionTypeChanged();
      });

      it('sets the log protocol to network file system', function() {
        expect(vm.scheduleModel.log_protocol).toEqual('Network File System');
      });
    ...
    describe('when the action type is automation_request', function() {
      ...
      it('sets the scheduleModel with the Automation params', function() {
        expect(vm.scheduleModel.instance_names).toEqual(data.instance_names);
        expect(vm.scheduleModel.starting_object).toEqual(data.starting_object);
        expect(vm.scheduleModel.instance_name).toEqual(data.instance_name);
        expect(vm.scheduleModel.object_message).toEqual(data.object_message);
        expect(vm.scheduleModel.object_request).toEqual(data.request);
        expect(vm.scheduleModel.target_class).toEqual(data.target_class);
        expect(vm.scheduleModel.ui_attrs).toEqual(data.ui_attrs);
        expect(vm.scheduleModel.target_id).toEqual(data.target_id);
      });
    });

    describe('when the action type is not db_backup', function() {
      beforeEach(function() {
        vm.scheduleModel.action_typ = 'not_db_backup';
        vm.scheduleModel.filter_typ = 'not_all';
        vm.filterValuesEmpty = false;
        vm.actionTypeChanged();
      });
    ...
    describe('when the action type is db_backup', function() {
      ...
      describe('when the log protocol is Samba', function() {
        ...
        it('returns true', function() {
          expect(vm.sambaBackup()).toBe(true);
        });
      });

      describe('when the log protocol is not Samba', function() {
        it('returns false', function() {
          expect(vm.sambaBackup()).toBe(false);
        });
      });
    });
  ...
  describe('#scheduleTimerTypeChanged', function() {
    describe('when the timer type is changed to once', function() {
      beforeEach(function() {
        ...
        vm.scheduleModel.timer_value = 'not null';
        $scope.angularForm = {
          timer_value: {
            $name:       'timer_value',
            $setViewValue: function (value){}
        ...
        vm.scheduleTimerTypeChanged();
      });

      it('sets the scheduleTimerValue to zero', function() {
        expect(vm.scheduleModel.timer_value).toEqual(0);
      });
    ...
    describe('when the timer type is changed to anything else', function() {
      beforeEach(function() {
        vm.scheduleModel.timer_typ = 'Hourly';
        vm.scheduleModel.timer_value = null;
        $scope.angularForm = {
          timer_value: {
            $name:       'timer_value',
            $setViewValue: function (value){}
      ...
      it('sets the scheduleTimerValue to 1', function() {
        expect(vm.scheduleModel.timer_value).toEqual(1);
      });
  ...
  describe('#timerNotOnce', function() {
    describe('when the timer type is once', function() {
      ...
      it('returns false', function() {
        expect(vm.timerNotOnce()).toBe(false);
      });
    });
    ...
    describe('when the timer type is not once', function() {
      ...
      it('returns true', function() {
        expect(vm.timerNotOnce()).toBe(true);
      });
    });
  });
  ...
  describe('Validates credential fields', function() {
    beforeEach(inject(function($compile, miqService) {
      ...
      var element = angular.element(
        ...
        '<input ng-model="vm.scheduleModel.depot_name" name="depot_name" required" text />' +
        '<input ng-model="vm.scheduleModel.uri" name="uri" required text />' +
        '<input ng-model="vm.scheduleModel.log_userid" name="log_userid" required text />' +
        '<input ng-model="vm.scheduleModel.log_password" name="log_password" required text />' +
        '</form>'
      );
    ...
    it('returns true if all the Validation fields are filled in and dirty', function() {
      expect(vm.canValidate()).toBe(true);
    });
  });
});
