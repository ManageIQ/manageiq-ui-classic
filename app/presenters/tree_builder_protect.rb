class TreeBuilderProtect < TreeBuilder
  ...
  def x_get_tree_hash_kids(parent, count_only)
    nodes = parent[:nodes].map do |policy|
      icon = policy.resource_type.safe_constantize.try(:decorate).try(:fonticon)
      {
        :id           => "policy_#{policy.id}",
        :text         => prefixed_title("#{ui_lookup(:model => policy.resource_type)} #{policy.mode.capitalize}", policy.description),
