ManageIQ.angular.app.component('ansibleCredentialOptions', {
  ...
  template: [
    ...
    '      <input ng-if="!attr.multiline" type="password" autocomplete="off" value="{{vm.storedPasswordPlaceholder}}" class="form-control" title="{{ __(attr.help_text) }}" ng-disabled="true" ng-hide="vm[name] || vm.newRecord">',
    ...
    '      <input ng-if="!attr.multiline" type="password" autocomplete="off" class="form-control" title="{{ __(attr.help_text) }}" ng-hide="!vm[name] && !vm.newRecord" auto-focus="reactiveFocus" ng-model="vm.model[name]">',
