...
ManageIQ.angular.app.component('repositoryForm', {
  bindings: {
    repositoryId: '@',
  },
  controllerAs: 'vm',
  controller: ['miqService', 'API', function (miqService, API) {
    ...
    this.$onInit = function () {
      ...
      vm.newRecord = this.repositoryId === 'new';
      ...
      if (this.repositoryId !== 'new') {
        API.get('/api/configuration_script_sources/' + this.repositoryId + '?attributes=' + vm.attributes.join(','))
    ...
    vm.saveClicked = function () {
      miqService.sparkleOn();
      API.put('/api/configuration_script_sources/' + this.repositoryId, vm.repositoryModel)
        .then(vm.getBack)
        .catch(miqService.handleFailure);
    };
  ...
  templateUrl: '/static/ansible/repository_form.html.haml',
});
