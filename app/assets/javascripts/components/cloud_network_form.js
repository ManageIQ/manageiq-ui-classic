ManageIQ.angular.app.component('cloudNetworkForm', {
  ...
  templateUrl: '/static/cloud_network/change.html.haml',
  bindings: {
    'cloudNetworkFormId': '@',
...
cloudNetworkFormController.$inject = ['miqService', 'API'] 
	
function cloudNetworkFormController(miqService, API) {
  var vm = this;
  ...
  var init = function() {
    ...
    if (vm.newRecord) {
      ...
      API.get("/api/cloud_networks/" + vm.cloudNetworkFormId + "?attributes=cloud_tenant,ext_management_system.name").then(function(data) {
