class ApplicationHelper::Toolbar::DriftCenter < ApplicationHelper::Toolbar::Basic
  button_group('drift_tasks', [
                 twostate(
                   ...
                   :url       => "drift_all",
                   :url_parms => proc { "?compare_task=all&db=#{@compare_db}&id=#{@drift_obj.id}" }
                 ),
                 twostate(
                   :drift_diff,
                   ...
                   :url_parms => proc { "?compare_task=different&db=#{@compare_db}&id=#{@drift_obj.id}" }
                 ),
                 twostate(
                   :drift_same,
                   ...
                   :klass     => ApplicationHelper::Button::ButtonWithoutRbacCheck,
                   :url       => "drift_same",
                   :url_parms => proc { "?compare_task=same&db=#{@compare_db}&id=#{@drift_obj.id}" }
                 ),
               ])
