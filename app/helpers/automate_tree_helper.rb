module AutomateTreeHelper
  def at_tree_select_toggle(edit_key)
    ...
    render :update do |page|
      ...
      when 'submit'
        ...
        if edit_key == :method
          @edit[:new][:embedded_methods] ||= []
          if !@edit[:new][:embedded_methods].include?(@edit[:automate_tree_selected_path])
            if @edit[:new][:embedded_methods].count >= 20
              #add_flash(_("You can have only 20 embedded methods at most"), :warning)
            elsif MiqAeMethod.find_by_id(@edit[:ae_method_id]).fqname == @edit[:automate_tree_selected_path]
              #add_flash(_("fqname"), :warning)
            else
  ...
  def at_tree_select(edit_key)
    ...
    if params[:id].start_with?("aei-")
      ...
      record = MiqAeMethod.find_by_id(id)
    end
    if edit_key != :method
