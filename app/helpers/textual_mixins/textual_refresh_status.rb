module TextualMixins::TextualRefreshStatus
  ...
  def refresh_status(record)
    ...
    refresh_time = record.last_refresh_date&.in_time_zone(Time.zone)
    if refresh_time
    ...
    end
    {
      ...
      :stale  => refresh_time&.before?(2.days.ago) # checking if date is within last two days range.
    }
  end

  def refresh_date(record)
    last_refresh_date = record.last_refresh_date&.in_time_zone(Time.zone)
