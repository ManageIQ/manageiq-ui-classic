class FirmwareController < ApplicationController
  ...
  include_concern 'FirmwareRegistries'
  ...
  FIRMWARE_X_BUTTON_ALLOWED_ACTIONS = {
    'firmware_registry_new'     => :firmware_registry_new,
    'firmware_registry_edit'    => :firmware_registry_edit,
    'firmware_registry_delete'  => :firmware_registry_delete,
    'firmware_registry_refresh' => :firmware_registry_refresh,
  }.freeze

  def x_button
    generic_x_button(FIRMWARE_X_BUTTON_ALLOWED_ACTIONS)
  end
  ...
  def features
    [
      {
        :role     => "firmware_registry",
        :role_any => true,
        :name     => :firmware_registries,
  ...
  def get_node_info(node, show_list = true)
    ...
    case x_active_tree
    when :firmware_registries_tree then firmware_registry_get_node_info(node)
    end
  ...
  def replace_right_cell(options = {})
    ...
    trees = build_replaced_trees(replace_trees, %i[firmware_registries])
    ...
    case x_active_tree
    when :firmware_registries_tree
      presenter.update(:main_div, r[:partial => "firmware_registry_list"])
      right_cell_text = if nodetype == "root"
                          _("All Firmware Registries")
                        else
                          case nodetype
                          when 'ps'
                            if @ps.id.blank?
