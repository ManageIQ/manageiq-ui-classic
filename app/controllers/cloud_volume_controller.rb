class CloudVolumeController < ApplicationController
  ...
  def button
    ...
    if params[:pressed] == "cloud_volume_attach"
      ...
      @volume = CloudVolume.find_by(:id => checked_item_id)
  ...
  def cloud_volume_form_fields
    ...
    volume = CloudVolume.find_by(:id => params[:id])
    render :json => {
      :name => volume.name
    }
  end
  ...
  def attach_volume
    ...
    when "attach"
      options = form_params
      vm = VmCloud.find_by(:id => options[:vm_id])
  ...
  def create
    ...
    when "add"
      @volume = CloudVolume.new
      options = form_params
      cloud_tenant = CloudTenant.find_by(:id => options[:cloud_tenant_id])
  ...
  def backup_create_finished
    ...
    volume_id = session[:async][:params][:id]
    task = MiqTask.find(task_id)
    @volume = CloudVolume.find_by(:id => volume_id)
  ...
  def backup_restore
    ...
    when "restore"
      @backup = CloudVolumeBackup.find_by(:id => params[:backup_id])
