module MiqPolicyController::PolicyProfiles
  ...
  def profile_build_edit_screen
    ...
    policies.each { |p| @edit[:new][:policies][ui_lookup(:model => p.resource_type) + " #{p.mode.capitalize}: " + p.description] = p.id } # Build a hash for the members list box

    @edit[:choices] = {}
    MiqPolicy.all.each do |p|
      @edit[:choices][ui_lookup(:model => p.resource_type) + " #{p.mode.capitalize}: " + p.description] = p.id # Build a hash for the policies to choose from
    end
  ...
  def profile_get_info(profile)
    ...
    @profile_policies = @profile.miq_policies.sort_by { |p| [p.resource_type, p.mode, p.description.downcase] }
