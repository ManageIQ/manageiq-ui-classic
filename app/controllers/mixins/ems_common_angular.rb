  ...
  module EmsCommonAngular
    ...
    def get_task_args(ems)
      ...
      when 'ManageIQ::Providers::Vmware::InfraManager'
        connect_opts = [{:pass => password, :user => user, :ip => params[:default_hostname]}] if params[:cred_type] == "default"
        connect_opts = [{:pass => MiqPassword.encrypt(params[:console_password]), :user => params[:console_userid], :ip => params[:default_hostname]}] if params[:cred_type] == "console"
        connect_opts
