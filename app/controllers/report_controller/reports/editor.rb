module ReportController::Reports::Editor
  ...
  def miq_report_edit
    ...
    when "add", "save"
      ...
      if params[:button].nil? && params[:tab].nil?
        javascript_redirect(:controller => "report", :action => "react_edit", :id => params[:id])
        return
      end
      miq_report_add_edit
    end
  ...
  def react_available_fields
    ...
    fields = MiqExpression.reporting_available_fields(model, perf_interval)
  ...
  # GET /report/react_preview_data(/:id)
  # Returns column structure (and empty rows) for the report.
  # Full async row generation is a follow-up; for now we surface col_order.
  def react_preview_data
    id = params[:id]
    if id.blank? || id == "new"
      render :json => {:columns => [], :rows => [], :stub => true}
      return
    end

    assert_privileges("miq_report_edit")
    rpt = find_record_with_rbac(MiqReport, id)
    columns = Array(rpt.col_order)
    render :json => {:columns => columns, :rows => []}
  end
  ...
  # Extracted so react_form_data and copy_from both use the same transformation.
  def serialize_report_attrs(rpt)
    ...
    # MiqReport stores bare field names (e.g. "created_on") but the picker
    # identifies fields as "Model-created_on".
    prefixed_col_order = Array(rpt.col_order).map { |f| "#{rpt.db}-#{f}" }
    # sortby entries may carry a "__suffix" (e.g. "created_on__week").
    # Strip the suffix before prefixing so the base field matches available-fields items,
    # then re-append the suffix so buildInitialValues can extract it correctly.
    ...
    # Serialize filters as plain MiqExpression hashes so the React expression
    # editor can seed them via miqToRqb (the adapter handles the conversion).
...

    {
      ...
      :db_options    => db_opts,
      # Filters — serialised as MiqExpression hashes; miqToRqb converts them client-side
      :record_filter  => record_filter,
      :display_filter => display_filter,
      # Trend fields — reconstructed as "db-col" for the React trend column picker
