...
import {
  Form as PfForm, Button, TabContainer, TabContent, Spinner, TabPane, Nav, NavItem, MessageDialog, Icon,
...
import {
  ANSIBLE_FIELDS, DEFAULT_PLACEHOLDER, CATALOG_ITEM_PROPS,
} from './constants';
...
export class AnsibleCatalogItemForm extends React.Component {
  ...
  componentDidMount() {
    const {
      loadCatalogs, loadDialogs, loadRepos, loadCredentials, loadCloudTypes,
      loadCatalogItem, loadCloudCredential, region, catalogItemFormId, availableCatalogs,
    } = this.props;
    ...
    if (catalogItemFormId) {
      ...
      loadCatalogItem(catalogItemFormId).then(() => {
        ...
        const { catalogItem } = ansibleCatalog;
  ...
  submitForm = (values) => {
    ...
    const {
      name, description, display, long_description, service_template_catalog_id, // eslint-disable-line camelcase
    } = values;
    const provisionFields = {}; const
      retirementFields = {};
    ANSIBLE_FIELDS.forEach((field) => {
      ...
      if (isIdField) { // blank ids passed to api cause failures
        ['provision', 'retirement'].forEach((prefix) => {
          ...
          if (value && prefix === 'provision') {
            provisionFields[field] = value;
          } else if (value) {
            retirementFields[field] = value;
          }
        });
      } else {
                ...
                <TabContainer defaultActiveKey={1} id="ansibleCatalogForm">
                  <div>
                    ...
                    <TabContent animation>
                      ...
                      {/* Retirement tab */}
                      <TabPane eventKey={2} mountOnEnter>
                        <div>
