...
describe('InstanceForm Component', () => {
    ...
    fields: [
      {
        ...
        value: 'value1',
        value_collect: '',
        icons: ['ff ff-attribute', 'carbon--CloseOutline'],
        message: '',
      },
      {
        id: 2,
        ...
        value: '42',
        value_collect: '',
        icons: ['ff ff-attribute', 'ff ff-integer', 'carbon--CloseOutline'],
        message: '',
      },
    ],
    ...
    fields: [
      {
        ...
        value_max_retries: '3',
        value_max_time: '60',
        icons: ['ff ff-state', 'carbon--CloseOutline'],
        message: '',
      },
    ],
    is_state_class: true,
  ...
  describe('Add Instance', () => {
    ...
    it('should submit form with valid data', async() => {
      ...
      fetchMock.post(`/miq_ae_class/create_instance?class_id=${classId}`, {
        success: true,
      ...
      await waitFor(() => {
        expect(fetchMock.called(`/miq_ae_class/create_instance?class_id=${classId}`)).toBe(true);
      });
    });
  });
  ...
  describe('Edit Instance', () => {
    ...
    it('should update instance with valid data', async() => {
      ...
      const recordId = '456';
      fetchMock.get(`/miq_ae_class/instance_form_data/${recordId}`, mockInstanceData);
      fetchMock.post(`/miq_ae_class/update_instance/${recordId}`, {
        success: true,
      ...
      await waitFor(() => {
        expect(fetchMock.called(`/miq_ae_class/update_instance/${recordId}`)).toBe(true);
      });
    });
  });
  ...
  describe('Form Actions', () => {
    ...
    it('should reset name field to original value and call add_flash with warning', async() => {
      const user = userEvent.setup();
      const recordId = '456';
      fetchMock.get(`/miq_ae_class/instance_form_data/${recordId}`, mockInstanceData);
      ...
      // Change the name field so that Reset becomes enabled
      const nameInput = container.querySelector('#name');
      await user.clear(nameInput);
      await user.type(nameInput, 'changed_name');
      expect(nameInput.value).toBe('changed_name');

      // Reset button is now enabled — click it
      const resetButton = screen.getByRole('button', { name: /reset/i });
      expect(resetButton).not.toBeDisabled();
      await user.click(resetButton);

      // Name reverts to original value from mockInstanceData
      await waitFor(() => {
        expect(nameInput.value).toBe('test_instance');
      });

      // add_flash is called with 'warning' severity
