...
import schema from '../../components/automate-namespace-form/namespace-form.schema';
import NamespaceForm from '../../components/automate-namespace-form';
...
describe('Automate Namespace form component', () => {
  ...
  it('should correctly initialize the component', async(done) => {
    fetchMock
      .getOnce('/api/automate_namespaces/2222?attributes=name,description,enabled,source',
        {
    ...
    expect(wrapper.find('label').first().text()).toEqual('Fully Qualified Name');
    expect(wrapper.find('input').first().instance().value).toEqual('f');
    done();
  });

  it('should send correct data on submit', async(done) => {
    fetchMock.getOnce('/api/automate_namespaces/2222?attributes=name,description,enabled,source',
      {
        name: 's', description: 'z', enabled: true, source: 'user',
      }).postOnce('/api/automate_namespaces/2222', {});
