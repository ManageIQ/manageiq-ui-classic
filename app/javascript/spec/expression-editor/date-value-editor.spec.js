...
describe('DateValueEditor — specific mode (dateFormat "s")', () => {
  ...
  test.each([
    ...
    await waitFor(() => {
      expect(screen.getByTestId('date-picker-input').placeholder).toBe(placeholder);
    });
  });
});
...
describe('DateValueEditor — mode toggle button', () => {
  ...
  test.each([
    ['s', 'r'],
    ['r', 's'],
  ])('toggles from %s to %s', async(from, to) => {
    const onToggle = jest.fn();
    await renderEditor({ dateFormat: from, onToggleFormat: onToggle, handleOnChange: jest.fn() });
    fireEvent.click(document.querySelector('button.exp-date-toggle-btn'));
    expect(onToggle).toHaveBeenCalledWith(to);
  });
