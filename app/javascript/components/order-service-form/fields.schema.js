...
const sharedSchema = (field, refreshInProgress) => ({
  id: field.id,
  name: field.name,
  label: field.label,
  hideField: !field.visible,
  isRequired: field.required,
  isDisabled: refreshInProgress || field.read_only,
  description: field.description,
});

/** Function to build a text box. */
export const buildTextBox = ({
  field, validate, orderServiceConfig, refreshInProgress,
}) => {
  let component = {};
  ...
  if (field.options.protected) {
    component = {
      ...
      ...sharedSchema(field, refreshInProgress),
    };
  } else {
    component = {
...
export const buildTextAreaBox = ({
  field, validate, updateFormReference, refreshInProgress,
}) => ({
...
export const buildDropDownList = ({ field, validate, orderServiceConfig }) => {
  ...
  return {
    component: componentTypes.SELECT,
    ...sharedSchema(field, orderServiceConfig.refreshInProgress),
