...
const ToastItem = ({ toastNotification }) => {
  ...
  if (toastNotification.data.link) {
    return (
      ...
      >
        <div className="pull-right toast-pf-action miq-notification-toast-item-view-details-link-wrapper">
          <Link href={toastNotification.data.link} className="miq-notification-toast-item-view-details-link">{__('View details')}</Link>
        </div>
      </ActionableNotification>
    );
  }
