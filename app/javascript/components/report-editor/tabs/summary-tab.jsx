...
const normalizeSelected = (items = []) => items.map((item) => String(item));

const getSortOptions = (colOrder = [], labelMap = {}) => [
  [__('Nothing'), NOTHING],
  ...colOrder.map((fieldId) => [labelMap[fieldId] || fieldId, fieldId]),
];

const getNumericColumns = (colOrder = [], fieldMetadata = {}, labelMap = {}) => colOrder
  .filter((fieldId) => getColumnMeta(fieldMetadata, fieldId).numeric)
  .map((fieldId) => ({ fieldId, label: labelMap[fieldId] || fieldId }));

const getSuffixOptions = (fieldMetadata, fieldId) => {
  const suffixes = getColumnMeta(fieldMetadata, fieldId).break_suffixes || [];
  return [['', __('Original Value')], ...suffixes.map(([label, value]) => [String(value), label])];
};

const getFormatOptions = (fieldMetadata, fieldId) => {
  const formats = getColumnMeta(fieldMetadata, fieldId).available_formats || [];
  return [['_none_', __('None')], ['', __('Reset to Default')], ...formats.map(([label, value]) => [String(value), label])];
};

const renderCalculationTable = (title, columns, selectedMap, options, onToggle) => {
  if (columns.length === 0) {
                ...
                {options.map((option) => (
                  <TableCell key={option}>
                    <Checkbox
                      ...
                      onChange={(_, { checked }) => onToggle(fieldId, option, checked)}
                    />
                  </TableCell>
                ))}
              </TableRow>
...
const SummaryTab = () => {
  ...
  const sortOptions = getSortOptions(colOrder, labelMap);
  const numericColumns = getNumericColumns(colOrder, fieldMetadata, labelMap);

  const sortby1 = sortby[0] || NOTHING;
  const sortby2 = sortby[1] || NOTHING;
  ...
  const updateGrouping = (fieldId, option, checked) => {
    const current = normalizeSelected(formValues.col_options?.[fieldId]?.grouping);
  ...
  const pivotBy1 = pivotBy1Raw || NOTHING;
  const pivotBy2 = pivotBy2Raw || NOTHING;
  const pivotBy3 = pivotBy3Raw || NOTHING;
  ...
  return (
    <div>
        ...
        {hasPrimarySort && (
          <>
            <FormRow label={__('Sort Order')}>
              <Select id="summary-order" labelText="" hideLabel value={order} onChange={(e) => formOptions.change('order', e.target.value)}>
            ...
            <FormRow label={__('Show Sort Breaks')}>
              <Select id="summary-group" labelText="" hideLabel value={group} onChange={(e) => formOptions.change('group', e.target.value)}>
              ...
              <>
                <FormRow label={__('Hide Detail Rows')}>
                  <Toggle
                    ...
                    labelB={__('Yes')}
                    toggled={!!hideDetails}
                    onToggle={(checked) => formOptions.change('hide_details', checked)}
                    hideLabel
                  />
                </FormRow>
                {showBreakFormat && (
      ...
      {hasGroupBreaks && renderCalculationTable(
        ...
        numericColumns,
        Object.fromEntries(
          Object.entries(formValues.col_options || {}).map(([k, v]) => [k, v?.grouping || []]),
        ),
        GROUPING_OPTIONS,
        updateGrouping,
      )}
