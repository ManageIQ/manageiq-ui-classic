...
const NamespaceTreeModal = ({
  ...
  useEffect(() => {
    ...
    API.get('/api/automate_domains?expand=resources').then((apiData) => {
      const filteredResources = domainId
        ? apiData.resources.filter((d) => String(d.id) === String(domainId))
        : apiData.resources;
      const domainNodes = filteredResources.map((domain) => ({
        id: domain.id,
        name: domain.name,
        children: [],
  ...
  if (selectedNode) {
    ...
    domainFqname = selectedNode.metadata.domain_fqname;
    displayPath = includeDomainPrefix ? fqname : domainFqname;
  }

  const handleApply = () => {
    if (fqname) {
      onApply({ displayPath, fqname, domainFqname });
    }
    onClose();
  };
