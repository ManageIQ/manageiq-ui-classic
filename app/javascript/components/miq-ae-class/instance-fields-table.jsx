...
const InstanceFieldsTable = ({ fields, isStateClass, onEditField }) => {
  ...
  const rows = fields.map((field, index) => {
    const row = {
      ...
      value: {
        text: field.datatype === 'password' ? '********' : (field.value || ''),
      },
    };
  ...
  const onCellClick = (item, action) => {
    if (action === CellAction.buttonCallback && typeof item.callbackAction === 'function') {
      item.callbackAction();
    }
  };

  return (
    <div className="ae-instance-fields-table">
      <MiqDataTable
        headers={headers}
        rows={rows}
