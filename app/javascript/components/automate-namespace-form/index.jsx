...
const getInitialValues = id => API.get(`/api/automate_namespaces/${id}?attributes=name,description,enabled,source`);

const handleSubmit = (id, parentId, values, redirectUrl) => {
  miqSparkleOn(); // miqRedirectBack turns it off
  const action = id ? 'edit' : 'create';
  const flashMessage = sprintf(id
    ? __('Automate Namespace "%s" was saved')
    : __('Automate Namespace "%s" was added'), values.name);

  API.post(`/api/automate_namespaces/${id || ''}`, {
...
const NamespaceForm = ({ id, parentId, namespacePath }) => {
  ...
  const cancelMessage = state.name
    ? sprintf(__('Editing "%s" Automate Namespace was cancelled by user.'), state.name)
    : __('Creating Automate Namespace was cancelled by user.');
  ...
  useEffect(() => {
    if (!state.loaded && id) {
      ...
      getInitialValues(id).then(({
        ...
        const readonly = ['user', 'user_locked'].includes(source); // Not working the way it should
        setState({
  ...
  return (
    <MiqFormRenderer
      initialValues={{
        namespace: namespacePath, name: state.name, description: state.description, enabled: state.enabled,
      }}
      schema={schema(state.readonly)}
      onSubmit={values => handleSubmit(id, parentId, values, redirectUrl)}
...
NamespaceForm.propTypes = {
  id: PropTypes.number, // If undefined or null then renders the new namespace form
