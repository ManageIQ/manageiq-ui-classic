...
import { PaginationRow, paginate, PAGINATION_VIEW, Spinner } from 'patternfly-react';
import { fetchCustomEventsIfNeeded } from '../redux/actions';
...
class CustomEventsTable extends Component {
  constructor(props) {
    super(props);
    this.state = {
      pagination: {
  ...
  componentDidMount() {
    this.props.fetchCustomEventsIfNeeded(this.props.userId);
  }
  ...
  render() {
    ...
    if (!events) return <div><Spinner loading size="lg" /></div>;
    const {
...
CustomEventsTable.defaultProps = {
  events: undefined,
};

const mapStateToProps = (
  { usersReducer: { userCustomEvents } },
  { match: { params: { userId } } },
) => ({
  events: userCustomEvents[userId],
  userId,
});
...
export default connect(mapStateToProps, mapDispatchToProps)(CustomEventsTable);
