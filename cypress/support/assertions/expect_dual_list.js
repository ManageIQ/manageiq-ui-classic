 ...
 *
 * @param {string} sourceList - The source list ('left' or 'right')
 * @param {string} targetList - The target list ('left' or 'right')
 * @param {string} targetItem - The text of the target item to select and move
 * @returns {void}
 */
function selectAndMoveItem(sourceList, targetList, targetItem) {
  const sourceIndex = sourceList === 'left' ? 'first' : 'last';
  const targetIndex = targetList === 'left' ? 'first' : 'last';
  const moveButton = sourceList === 'left' ? '#move-right' : '#move-left';

  // TODO: replace with dualListAction once #9654 is merged
  // Select the item
  cy.get(DUAL_LIST_BODY_SELECTOR)
    [sourceIndex]()
    .contains(DUAL_LIST_ROW_SELECTOR, targetItem)
    .click();
  cy.get(moveButton).should('be.enabled');
  // Move the item
 ...
 *
 * @param {string} listIndex - The list to verify ('first' for left list, 'last' for right list)
 * @param {string[]} listItems - Array of list items to verify
 * @returns {Cypress.Chainable} - The Cypress chainable object
 */
function verifyListItems(listIndex, listItems) {
  return cy
 ...
 * @returns {void}
 */
function moveAllItems(sourceList, targetList, listItems) {
  // TODO: replace with dualListAction once #9654 is merged
  const moveAllButton =
    sourceList === 'left' ? '#move-all-right' : '#move-all-left';
  // Move all items
  cy.get(moveAllButton).click();

  const sourceIndex = sourceList === 'left' ? 'first' : 'last';
  const targetIndex = targetList === 'left' ? 'first' : 'last';
  // Verify all items are in the target list
  verifyListItems(targetIndex, listItems);
...
function testDualListStartingFromAvailableItems(initialAvailableItems) {
  ...
  selectAndMoveItem('left', 'right', initialAvailableItems[0]);
  selectAndMoveItem('right', 'left', initialAvailableItems[0]);
  // Test moving all items from left to right
  moveAllItems('left', 'right', initialAvailableItems);
  ...
  moveAllItems('right', 'left', initialAvailableItems);
...
function testDualListStartingFromSelectedItems(initialSelectedItems) {
  ...
  selectAndMoveItem('right', 'left', initialSelectedItems[0]);
  selectAndMoveItem('left', 'right', initialSelectedItems[0]);
  // Test moving all items from right to left
  moveAllItems('right', 'left', initialSelectedItems);
  // Test search functionality on left list
  testSearchFunctionality('left', initialSelectedItems[0]);
  // Test moving all items from left to right
  moveAllItems('left', 'right', initialSelectedItems);
  // Test search functionality on right list
...
Cypress.Commands.add(
  ...
  ({
    ...
    if (availableItems.length) {
      ...
      verifyListItems('first', availableItems);
      ...
      verifyListItems('last', selectedItems);
